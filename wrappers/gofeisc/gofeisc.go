...
func DeleteReader(readerHnd int) (result int, err error) {
	result, err = feisc.FEISC_DeleteReader(readerHnd)
	return
}
...
func GetLastError(readerHnd int) (errorCode int, errorText string, result int, err error) {
	text := make([]byte, 256)
	result, err = feisc.FEISC_GetLastError(readerHnd, &errorCode, &text[0])
...
func GetReaderParam(readerHnd int, param string) (value string, result int, err error) {
	val := make([]byte, 128)
	result, err = feisc.FEISC_GetReaderPara(readerHnd, &([]byte(param))[0], &val[0])
...
func SetReaderParam(readerHnd int, param string, value string) (result int, err error) {
	result, err = feisc.FEISC_SetReaderPara(readerHnd, &([]byte(param))[0], &([]byte(value))[0])
	return
}
