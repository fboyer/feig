...
func FEISC_GetLastState(iReaderHnd int, cStatusText *byte) (result int, err error) {
	r0, _, e1 := syscall.Syscall(procFEISC_GetLastState.Addr(), 2, uintptr(iReaderHnd), uintptr(unsafe.Pointer(cStatusText)), 0)
	result = int(r0)
	if result < 0 {
		if e1 != 0 {
			err = error(e1)
